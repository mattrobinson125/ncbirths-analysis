...
\newpage
## Mother's Marital Status, Race, and Age
RaceMom Factor
1=white, 2=black, 3=American Indian, 4=Chinese, 5=Japanese, 6=Hawaiian, 7=Filipino, or 8=Other 
```{r, include = FALSE}
...
It should be noted that there are many missing values throughout the tables and analysis presented, which indicates an absence of data. For example, there is insufficient data on hawaiian individuals to apply a regression model for this specific subset. These missing values are easy easily overlooked in the midst of other discussion- but they indicate an underlying problem throughout the dataset. 

Additionally, it is easy and tempting to draw braod conclusions based on statistical analysis and tests like those applied earlier. However, it is important to remember that observing a trend does not allow mathematicians to draw conclusions about why or how those patterns emerge. For example, one might note that unmarried women tend to have children at a younger age than married women within the same racial subset- however, not conclusions / assumptions should be drawn based on this observaton, especially at such initial stages of data analysis. Additionally, there is insufficient data to say that the observations made here are significant, or could apply to a larger population (the data here represents a convenience sample, and does not accurately depict the entire population). 
