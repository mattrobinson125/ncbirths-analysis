...
Notes: This graphic illustrate the pitfalls of conducting a convenience sample. The groups represented clearly neglect certain subsets of the population (for example, there hawaiian individuals are not represented in the dataset). There is one data point for the filipino race- this means that the entire set of statistics computed for this subset is based on one individual. It is important to keep this in mind when interpreting any statistics computed 

It is also fascinating to look at this distribution, once it has been factored by marital status. We can dive into this exploration further, by comparing means of different subsets within the population. 
...
TTest P-Values
Difference in mom age- comparing racial groups to the entire population 
...
The pvalues computed, and the conclusions we draw from these pvalues, provide interesting insight into the dataset. For example, the japanese subset is the only population within which the age of married vs. not-married mothers is not statistically significant. The power of the T-Test is its ability to make sense of large datasets, and to determine which differences are meaningful (as opposed to natural variation within the population). 

Playing around with some new regression code:

It would be very interesting to build a repression predicting MomAge from MomRace and Marital Status. This regression is based around an interaction of variables, and will result in B values for each possible combination (example- white married, white not married, black married, black not married...). The regression will be centered around (MomAge - Population_mean_age), the difference between MomAge for a specific subset, and the overall population average age. This regression does not require an intercept (because it is categorical, not continuous data). 
...
The coefficients produced from this regression tell an intriguing story about the interaction that occurs between these two sociological variables (race and marital status), and the influence this interaction has when predicting MomAge.
...
## Marital Status, Age of Mother, and Weight Gained
...
```{r,fig.height = 3, fig.width = 3.6,echo=FALSE}
ggplot(NCbirths1, aes(MomAge, Gained, color=factor(Marital))) + geom_point(size=1) + labs(color="Marital")+geom_smooth(method = "lm") +ggtitle("NCbirths")
...
ggplot(NCbirths1, aes(MomAge, Gained, color=factor(Marital))) + geom_point(size=1) + labs(color="Marital")+geom_vline(xintercept = 27) + ggtitle("NCbirths1 with x-intercept of x = 27")
...
fm<-lm(Gained~MomAge*Marital,data=NCbirths1)
