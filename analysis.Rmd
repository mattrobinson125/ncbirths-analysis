...
marriedLM <- lm(married ~ NCbirths1$BirthWeightOz, multi)
table(married, multi)
```
...
qplot(factor(marital_cat), BirthWeightOz, data = NCbirths1, geom = "boxplot", color = factor(multi))
tapply(NCbirths1$BirthWeightOz, married, FUN = mean)
qplot(factor(Plural), BirthWeightOz, data = NCbirths1, geom = "boxplot" , color=factor(Marital))
tapply(NCbirths1$BirthWeightOz, multi, FUN = mean)
...
Given the amount of data that we had on plural births, we cannot draw any conclusions between the relationship of BirthWeightOz and Multiple Births given the marital status of the mother. Even though the data shows a trend that a non-married mother will birth a single child that has a lower weight average compared to that of a married mother, which is inline with our initial hypothesis. There is simply not enough data to say the same about twins or even triplets. The lack of data makes it difficult to draw any concrete conclusions past a single child birth.  
